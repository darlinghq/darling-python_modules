...
FT2Font::draw_glyphs_to_bitmap(const Py::Tuple &args, const Py::Dict &kwargs)
{
    ...
    for (size_t n = 0; n < glyphs.size(); n++)
    {
        ...
        if (error)
        {
            char buf[128]; sprintf(buf, "FT2Font::draw_glyphs_to_bitmap: Could not convert glyph to bitmap: %d", error); throw Py::RuntimeError(buf);
        }
...
FT2Font::get_xys(const Py::Tuple &args, const Py::Dict &kwargs)
{
    ...
    for (size_t n = 0; n < glyphs.size(); n++)
    {
        ...
        if (error)
        {
            char buf[128]; sprintf(buf, "FT2Font::get_xys: Could not convert glyph to bitmap: %d", error); throw Py::RuntimeError(buf);
        }
...
FT2Font::draw_glyph_to_bitmap(const Py::Tuple &args, const Py::Dict &kwargs)
{
    ...
    if (error)
    {
        char buf[128]; sprintf(buf, "FT2Font::draw_glyph_to_bitmap: Could not convert glyph to bitmap: %d", error); throw Py::RuntimeError(buf);
    }
